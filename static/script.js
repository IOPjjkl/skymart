    ...
    if (roomName) {
        ...
        try {
            ...
            answers.forEach((answer, index) => {
                ...
                const formattedImage = formatAndDisplayImage(answer.image_url);
                const answerHtml = `
...
// Функция для форматирования и отображения изображения
function formatAndDisplayImage(imageUrl) {
    if (imageUrl) {
        // Если есть ссылка на изображение, отображаем его
        return `<img src="${imageUrl}" class="max-w-full h-auto mb-4" alt="Изображение">`;
    } else {
        // Если нет ссылки на изображение, возвращаем пустую строку
        return '';
    }
}
